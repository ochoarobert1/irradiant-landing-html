...
function documentCustomLoad() {
    ...
    var imageClick = document.getElementsByClassName('image');
    var video = document.getElementById('video');
    var heightDiv = document.getElementsByClassName('main-item-wrapper-1');

    var fixedHeight = heightDiv[0].clientHeight;

    var changeDiv = document.getElementsByClassName('main-item-wrapper');

    for (var i = 0; i < changeDiv.length; i++) {
        if (!changeDiv[i].classList.contains('main-item-wrapper-1')) {
            changeDiv[i].setAttribute("style", "height:" + fixedHeight + 'px');
            changeDiv[i].classList.add('animate__animated', 'animate__fadeIn');
            changeDiv[i].classList.remove('d-none');
        } else {
            changeDiv[i].style.removeProperty('visibility');
        }
    }

    imageClick[0].addEventListener('click', function () {
        modal[0].classList.toggle('modal-hidden');
    }, false);
...
window.onload = window.onresize = function () {
    setTimeout(function () {
        var heightDiv = document.getElementsByClassName('main-item-wrapper-1');
        var fixedHeight = heightDiv[0].offsetHeight + 1;
        var changeDiv = document.getElementsByClassName('main-item-wrapper');
        for (var i = 0; i < changeDiv.length; i++) {
            if (!changeDiv[i].classList.contains('main-item-wrapper-1')) {
                changeDiv[i].setAttribute("style", "height:" + fixedHeight + 'px');
